...
async def get_todo(session: AsyncSession, user_id: int, todo_id: int) -> Todo | None:
...
async def get_user(session: AsyncSession, username: str) -> User | None:
    stmt = select(User).options(selectinload(User.todos)).where(User.username == username)
    result = await session.execute(stmt)
    return result.scalar_one_or_none()
...
async def get_user_session(
    ...
    stmt = select(User).options(selectinload(User.todos)).where(User.access_token == access_token)
    result = await session.execute(stmt)
    return result.scalar_one_or_none()
